...
class Block(nn.Module):
    '''
    Layer Norm - Attention - drop path for stochastic depth - Residual Connection
    Layer Norm - MLP - drop path for stochastic depth - Residual Connection
    '''
...
class PatchEmbed(nn.Module):
    """ 
    Image to Patch Embedding (Flattened)
    """
    ...
    def forward(self, x):
        ...
        return x # [N x imagesize x embedding dim]


class HybridEmbed(nn.Module):
    """ 
    CNN Feature Map Embedding
    Extract feature map from CNN(backbone), flatten, project to embedding dim.
    """
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if isinstance(img_size,tuple):
            self.img_size = img_size # Based on ImageNet Pretraining
        else:
        ...
        # Learnable Token and Positional Embedding
    ...
    def finetune_det(self, img_size=[800, 1344], det_token_num=100, mid_pe_size=None, use_checkpoint=False):
        '''
        Add Det token
        Change Patch Positonal Embedding (for patch) by interpolation (224 -> 800)
        Positional Embedding -> [Cls + Patch + Det]
        may have mid positional embedding
        '''
        ...
        patch_pos_embed = self.pos_embed[:, 1:, :] # 0 is cls
    ...
    def InterpolateInitPosEmbed(self, pos_embed, img_size=(800, 1344)):
        '''
        Imagesize may be different than (800,1344) -> interpolate
        Returns:
            iterpolated positional emebdding
        '''
        # import pdb;pdb.set_trace()
    ...
    def forward_features(self, x):
        '''
        Parameters:
            x: tensor batch of image 
        Returns:
            x: [N x Det Token x Feature dim]
        '''
        # import pdb;pdb.set_trace()
        ...
        for i in range(len((self.blocks))):
            if self.use_checkpoint:
                ...
                x = self.blocks[i](x)
            if self.has_mid_pe:
                if i < (self.depth - 1): # add mid pos embedding on last block
    ...
    def forward_return_all_selfattention(self, x):
        ...
        x = x + temp_pos_embed # add positional embedding
        x = self.pos_drop(x)
        # Transformer
        output = []
        for i in range(len((self.blocks))):
            ...
            if self.has_mid_pe:
                if i < (self.depth - 1):
                    x = x + temp_mid_pos_embed[i] # add mid pos embedding on last block
